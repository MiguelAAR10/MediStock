...
def crear_paciente():
    ...
    try:
        curation_result = DataCurationService.curate_paciente_payload(
            json_data,
            require_nombre=True,
            require_dni=True,
        )
        ...
        data_validada = create_schema.load(curation_result["curated"])
        ...
        return APIResponse.success(
            data=result, message="Paciente creado exitosamente", status_code=201
        )

    except ValidationError as e:
        # Error: El JSON estaba mal formado (ej: DNI con letras)
        return APIResponse.error(
            "Error de validación", status_code=400, details=e.messages
        )
...
def get_pacientes():
    ...
    try:
        ...
        resultado = PacienteService.listar_pacientes(
            page=page, per_page=per_page, search=search, distrito_id=distrito_id
        )
...
def update_paciente(id_paciente):
    ...
    try:
        ...
        data_validada = update_schema.load(curation_result["curated"], partial=True)

        if not data_validada:
            return APIResponse.error(
                "Sin datos válidos para actualizar", status_code=400
            )

        paciente_actualizado = PacienteService.actualizar_paciente(
            id_paciente, data_validada
        )
